/*
Orteil's sloppy IDK Clicker dungeons
...
var LaunchDungeons = function () {
  ...
  var basicLoot = { idk: { min: 1, max: 5, prob: 0.5 } };
  var goodLoot = { idk: { min: 3, max: 8, prob: 1 }, gear: { prob: 0.05 } };
  var bossLoot = { gear: { prob: 1 } };
  var chestLoot = { idk: { min: 2, max: 20, prob: 1 }, gear: { prob: 0.1 } };
  var bossLoot = { idk: { min: 10, max: 50, prob: 1 }, gear: { prob: 0.2 } };
  ...
  Game.Entity = function (
    ...
    if (this.type == "monster") {
      ...
      this.onKill = function () {
        if (this.subtype == "random") {
          ...
          if (value > 0) {
            var entity = this.dungeon.AddEntity("item", "idk", this.x, this.y);
            entity.value = value;
          }
        }
      };
    ...
    this.Draw = function () //return the string to draw this
    {
      ...
      if (this.type == "item" && this.subtype == "idk" && this.value > 0) {
        ...
        if (this.stats.hp <= 0) {
          ...
          if (this.type == "monster" && by.type == "hero") {
            ...
            if (Game.Monsters[this.subtype].loot) {
              ...
              if (
                loot.idk &&
                (!loot.idk.prob || Math.random() < loot.idk.prob)
              ) {
                ...
                entity.value = Math.round(
                  loot.idk.min + Math.random() * (loot.idk.max - loot.idk.min)
                );
              }
            }
            if (this.onKill) this.onKill();
    ...
    this.Turn = function () //do this every turn (walk around, heal up...)
    {
      ...
      if (this.type == "hero") {
        //collect items and idk
        var entities = this.dungeon.GetEntities(this.x, this.y);
        for (var i in entities) {
          if (entities[i].type == "item" && entities[i].subtype == "idk") {
        ...
        if (tile != -1) {
          ...
          if (Math.random() < 0.2) {
            ...
            if (Math.random() < 0.6) {
              ...
              if (value > 0) {
                var entity = this.AddEntity("item", "idk", tile[0], tile[1]); //random idk
                entity.value = value;
              }
  ...
  hero.dialogue = {
    ...
    completion:
      "Oh... oh my.|That's... I uh, I'm glad.|Y-yeah that was real easy. Piece of pie!|T-too easy, right?|S-so many idk...|Ooh? F-fascinating.",
    defeat:
